...
# -----------------------------------------------------------------------------
# POSMLP_VIDEO options
# -----------------------------------------------------------------------------
_C.POSMLP_VIDEO = CfgNode()

# layers.
_C.POSMLP_VIDEO.LAYERS = [4, 3, 8, 3]

# whether downsample in the end.
_C.POSMLP_VIDEO.TRANSITIONS = [True, False, False, False]

# dimesion of segment.
_C.POSMLP_VIDEO.SEGMENT_DIM = [32, 16, 16, 16]

# temporal stride.
_C.POSMLP_VIDEO.T_STRIDE = 1

# mlp ratio of different layer.
_C.POSMLP_VIDEO.MLP_RATIOS = [3, 3, 3, 3]

# embedding dimensions of different layer.
_C.POSMLP_VIDEO.EMBED_DIMS = [192, 384, 384, 384]

# patch size.
_C.POSMLP_VIDEO.PATCH_SIZE = 7

# override default qk scale of head_dim ** -0.5 if set.
_C.POSMLP_VIDEO.QKV_SCALE = None

# enable bias for qkv if True.
_C.POSMLP_VIDEO.QKV_BIAS = True

# attention dropout rate.
_C.POSMLP_VIDEO.ATTENTION_DROPOUT_RATE = 0

# stochastic depth rate.
_C.POSMLP_VIDEO.DROP_DEPTH_RATE = 0.1

# pretrained path
_C.POSMLP_VIDEO.PRETRAIN_PATH = None

#shift
_C.POSMLP_VIDEO.SHIFT = False

_C.WIN_SIZE = [14,14,14,7]
_C.WIN_SIZE_T = [8,3,3,3]
_C.DEPTH = [2,2,18,2]
_C.EMBED_DIM_TS = [112, 224, 392, 784]
_C.POSGAMMA = [8,16,32,64]
...
_C.MODEL.SINGLE_PATHWAY_ARCH = ["2d", "c2d", "i3d", "slow", "x3d", "mvit", "pos"]
